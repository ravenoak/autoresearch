...
[Go Task](https://taskfile.dev/). Running `task install` automatically
downloads `task` to `.venv/bin` when it's missing; the same check occurs in
`./scripts/setup.sh` during a full developer bootstrap. To install Go Task
manually use:
