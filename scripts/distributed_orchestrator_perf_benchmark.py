...
Usage:
    uv run scripts/distributed_orchestrator_perf_benchmark.py --max-workers 4 \
        --tasks 100 --network-latency 0.005
"""
...
def run_benchmark(
    max_workers: int, tasks: int, network_latency: float = 0.005
) -> List[Dict[str, float]]:
    ...
    """
    if max_workers <= 0 or tasks <= 0 or network_latency < 0:
        raise ValueError("max_workers and tasks must be positive; latency must be >= 0")

    results: List[Dict[str, float]] = []
    for workers in range(1, max_workers + 1):
        metrics = distributed_orchestrator_sim.run_simulation(
            workers=workers, tasks=tasks, network_latency=network_latency
        )
        results.append(
            {
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Distributed orchestrator performance benchmark")
    ...
    results = run_benchmark(args.max_workers, args.tasks, args.network_latency)
