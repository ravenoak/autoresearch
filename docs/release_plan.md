...
confirmed in `pyproject.toml` and [installation.md](installation.md). Loading
the PATH helper generated by `./scripts/setup.sh --print-path` exposes the Go
Task CLI in fresh shells, so `task --version` reports 3.45.4 immediately.
【5d8a01†L1-L2】 Running `uv run python scripts/check_env.py` continues to show
the expected toolchain—including Go Task, Black, Flake8, and Hypothesis—once
the `dev-minimal` and `test` extras are synced.
【0feb5e†L1-L17】【fa650a†L1-L10】 Storage regressions are under control:
...
【dbf750†L1-L1】 The xpass
comes from `tests/unit/test_storage_errors.py::test_setup_rdf_store_error`, so a
cleanup ticket tracks removal of the stale xfail before the release coverage
rerun. 【cd543d†L1-L1】【F:issues/remove-stale-xfail-for-rdf-store-error.md†L1-L27】
After syncing the docs extras, `uv run --extra docs mkdocs build` completes
without navigation warnings. 【e808c5†L1-L2】 The newest `task check` run now
fails during `scripts/lint_specs.py` because `docs/specs/monitor.md` and
...
template before re-running the suite.【4076c9†L1-L2】 The issue documents the
required fixes ahead of the next run.
【F:issues/restore-spec-lint-template-compliance.md†L1-L33】
