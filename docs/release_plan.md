...
the PATH helper generated by `./scripts/setup.sh --print-path` now exposes the
Go Task CLI in fresh shells, so `task --version` reports 3.45.4 immediately.
【c1ab5e†L1-L8】【5a32ba†L1-L3】 Running `uv run python scripts/check_env.py`
continues to show the expected toolchain—including Go Task, Black, Flake8, and
Hypothesis—once the `dev-minimal` and `test` extras are synced.
【582859†L1-L25】 Storage regressions are under control:
...
tests. 【b8e216†L1-L3】【babc25†L1-L3】 The xpass comes from
`tests/unit/test_storage_errors.py::test_setup_rdf_store_error`, so a cleanup
ticket tracks removal of the stale xfail before the release coverage rerun.
【9da781†L1-L3】【F:issues/remove-stale-xfail-for-rdf-store-error.md†L1-L27】
Integration ranking checks and distributed coordination properties still pass
with the `[test]` extras, and the VSS extension loader suite remains green
