...
_dummy_cfg = types.SimpleNamespace(
    search=types.SimpleNamespace(
        context_aware=types.SimpleNamespace(
            enabled=True, expansion_factor=0.5, max_history_items=10
        )
    )
)
...
def test_add_to_history_extracts_entities(
    ...
    monkeypatch.setattr("autoresearch.search.context.get_config", lambda: _dummy_cfg)
