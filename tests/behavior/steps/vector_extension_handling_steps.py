...
# Scenarios
@scenario("../features/vector_extension_handling.feature", "Load VSS extension from filesystem")
...
@scenario("../features/vector_extension_handling.feature", "Download VSS extension automatically")
...
def local_extension(tmp_path, monkeypatch):
    ...
    def mock_info(msg, *args, **kwargs):
        ...
        if str(extension_dir) in str(msg):
            original_info(f"Loading vss extension from filesystem: {extension_dir}", *args, **kwargs)
...
@given("I have an invalid local copy of the VSS extension", target_fixture="invalid_extension")
...
def check_loaded_from_filesystem(bdd_context):
    ...
    assert any(f"Loading vss extension from filesystem: {extension_path}" in log for log in logs), \
    ...
    assert any("VSS extension loaded successfully from filesystem" in log for log in logs), \
...
def check_vector_search_works(monkeypatch, bdd_context):
    ...
    try:
        ...
        with patch('autoresearch.storage.StorageManager.has_vss', return_value=True):
            ...
            mock_results = [
                {"node_id": "test1", "embedding": [0.1, 0.2, 0.3]}
            ]
