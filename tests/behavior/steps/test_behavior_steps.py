...
@when(
    parsers.re(
        r'I run `autoresearch\.search\("(?P<query>.+)"\)` via the MCP CLI'
    )
)  # noqa: E501
...
def check_agent_order_rotation(run_two_queries):
    ...
    assert (
        run_two_queries["primus_indices"][0]
        != run_two_queries["primus_indices"][1]
    )
...
@scenario(
    "../features/dkg_persistence.feature", "Persist claim in RDF quad-store"
)
...
@scenario(
    "../features/agent_orchestration.feature", "Rotating Primus across loops"
)
...
@scenario(
    "../features/output_formatting.feature", "Piped output defaults to JSON"
)
...
@scenario(
    "../features/reasoning_mode.feature", "Direct mode runs Synthesizer only"
)
