...
from autoresearch.storage import teardown, _db_conn, _graph, _rdf_store, _lru
        ...
        with patch('autoresearch.storage._graph', mock_graph):
            with patch('autoresearch.storage._rdf_store', mock_rdf):
                with patch('autoresearch.storage._lru', mock_lru):
                    ...
                    from autoresearch.storage import _db_conn, _graph, _rdf_store
...
def test_teardown_removes_db_file():
    ...
    with patch('autoresearch.storage._db_conn', mock_conn):
        with patch('autoresearch.storage._db_path', str(mock_path)):
            # Execute
