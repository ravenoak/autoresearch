...
def test_module_docstrings():
    ...
    for module, name in modules:
        ...
        assert len(docstring.strip().split('\n')) >= 1, f"{name} module docstring should have at least 1 line"

        # For all modules except __init__, check for more comprehensive docstrings
        if name != "llm":
            assert len(docstring.strip().split('\n')) >= 2, f"{name} module docstring should have at least 2 lines"
...
def test_class_docstrings():
    ...
    for cls in classes:
        ...
        assert len(docstring.strip().split('\n')) >= 1, f"{cls.__name__} docstring should have at least 1 line"

        # Get methods defined in the class itself
        class_methods = {name: method for name, method in inspect.getmembers(cls, inspect.isfunction)
        ...
        for name, method in class_methods.items():
            ...
            assert len(method_doc.strip().split('\n')) >= 1, f"{cls.__name__}.{name} docstring should have at least 1 line"
            ...
            if params:
                assert "Args:" in method_doc, f"{cls.__name__}.{name} docstring should have an Args section"

            # Check for Returns section if the method returns something
            # Skip __init__ methods as they implicitly return None
            if (sig.return_annotation is not inspect.Signature.empty and 
                sig.return_annotation is not None and 
                name != '__init__'):
                assert "Returns:" in method_doc, f"{cls.__name__}.{name} docstring should have a Returns section"

            # Check for Raises section if the method can raise exceptions
            if "raise" in inspect.getsource(method):
                assert "Raises:" in method_doc, f"{cls.__name__}.{name} docstring should have a Raises section"
...
def test_function_docstrings():
    ...
    for func in functions:
        ...
        assert len(docstring.strip().split('\n')) >= 1, f"{func.__name__} docstring should have at least 1 line"
        ...
        if sig.return_annotation is not inspect.Signature.empty and sig.return_annotation is not None:
            if "@contextmanager" in inspect.getsource(func) or "yield" in inspect.getsource(func):
                assert "Yields:" in docstring, f"{func.__name__} docstring should have a Yields section"
            else:
                assert "Returns:" in docstring, f"{func.__name__} docstring should have a Returns section"
